...
class Metric():
    ...
    # This only matters due to the queue system that moves 
    # objects between processes and can end up storing a large 
    # number of objects in the queue waiting for the handlers to flush
    __slots__ = [
    ...
    def __init__(self, path, value, raw_value=None, timestamp=None, 
        ...
        Takes:
            path=string: specifies the path of the metric
            value=[float|int]: the value to be submitted
            timestamp=[float|int]: in seconds since epoch
            precision=int: default is 2 and should work fine
        """
        ...
        # The value needs to be a float or an int. If it is, great. 
        # If not, try to cast it to one of those
    ...
    def getPathPrefix(self):
        ...
        i.e. servers.host.cpu.total.idle --> servers
        """
        # If we don't have a host name, assume it is just the
        # first part of the metric path
    ...
    def getCollectorPath(self):
        ...
        i.e. servers.host.cpu.total.idle --> cpu
        """

        # If we don't have a host name, assume it's just the 
        # third part of metric path
        if self.host is None:
    ...
    def getMetricPath(self):
        ...
        i.e. servers.host.cpu.total.idle --> total.idle
        """

        # If we don't have a host name, 
        # assume it is just the fourth+ of the metric path
        if self.host is None:
