...
def collector_process(collector, metric_queue, log):
    ...
    # Reset signal handlers of fork/threads
    ...
    # delay to avoid having all collectors running at the same time.
    next_window = math.floor(time.time() / interval) * interval
    stagger_offset = random.uniform(0, interval - 1)

    # Allocate time till the end of the window for the collector to run. 
    ...
    while (True):
        try: 
            ...
            if time_to_sleep > 0:
                ...
                # Clock ha jumped, lets skip missed intervals
                next_window = time.time()

            next_window += interval

            # Ensure that collector run times fit into the collection window
            ...
            # Adjust the stagger_offset to allow for more time to run
            # the colector
            ...
            # We must first disable the alarm as we don't want it to 
            # interrupt us and end up with a half of a loaded config
            signal.alarm(0)

            log.info('Reloading config file due to HUP')
